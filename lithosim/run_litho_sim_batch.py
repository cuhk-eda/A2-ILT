...
def run_litho_sim_batch():
    ...
    for path, subdirs, files in os.walk(image_root):
        for name in files:
            if fnmatch(name, '*.png') or fnmatch(name, '*.jpg'):
                ...
                _, bin_mask = litho.lithosim(image_data, threshold, kernels,
                                             weight, save_name, save_bin_wafer_image, kernel_number)
                print("------ Finish! ------\n")
    print("------ Done! ------")
